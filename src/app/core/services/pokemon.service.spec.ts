...
describe('PokemonService', () => {
  ...
  beforeEach(() => {
    ...
    service = new PokemonService(<never>httpClientSpy);
  });
  ...
  it('should return a pokemon pagination object', () => {
    const expectedPaginationPokemon: PaginationModel = new PaginationModel();
    httpClientSpy.get.and.returnValue(of(expectedPaginationPokemon));
    service.getPokemons('offset=0&limit=0').subscribe((pokemonPagination) => {
      expect(pokemonPagination).toEqual(
        expectedPaginationPokemon,
        'expected pokemon pagination'
      );
    });
  ...
  it('should return a type pagination object', () => {
    const expectedPaginationType: PaginationModel = new PaginationModel();
    httpClientSpy.get.and.returnValue(of(expectedPaginationType));
    service.getTypes().subscribe((typePagination) => {
      expect(typePagination).toEqual(
        expectedPaginationType,
        'expected type pagination'
      );
    });
    expect(httpClientSpy.get.calls.count()).toBe(1, 'one call');
  });
  ...
  it('should return a type object', () => {
    ...
    service.getTypes().subscribe((type) => {
